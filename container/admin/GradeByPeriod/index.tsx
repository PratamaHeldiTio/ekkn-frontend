...
import { IGradePeriodPage, IGroup } from "@/pages/sakera/grade/grade.types";
import dynamic from "next/dynamic";
import { useRouter } from "next/router";

const AdminLayout = dynamic(() => import("@/layout/AdminLayout"));

export default function OutputPeriod({ groups }: IGradePeriodPage) {
  const router = useRouter();
  return (
    <AdminLayout navigations={[{ title: "Kembali", link: "/sakera/grade" }]}>
      <div className="bg-secondary p-8 rounded-3xl my-8">
        <h1 className="text-2xl font-bold mb-12 ml-4">Nilai Mahasiswa</h1>
                    ...
                    <td className="px-6 py-4">
                      <div className="h-8 flex place-items-center">
                        <InputSubmit
                          value="Lihat detail"
                          onClick={() =>
                            router.push(`/sakera/grade/group/${group.id}`)
                          }
                        />
                      </div>
                    </td>
